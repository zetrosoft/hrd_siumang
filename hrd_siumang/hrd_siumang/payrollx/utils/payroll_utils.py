...
def calculate_pph21(doc, method=None):
	...
	try:
		...
		# 3. Dapatkan tarif TER dari Doctype 'Tarif Efektif Rata-rata'
		...
		print(
			f"DEBUG PPH: Karyawan: {employee.name}, Status PTKP: {status_ptkp}, Kategori TER: {kategori_ter}"
		)
		print(f"DEBUG PPH: Menggunakan metode PPh 21 TER dengan tarif: {ter_rate*100}%")

		# 4. Hitung PPh 21 Bulanan
		pph21_bulanan = total_pendapatan_bruto_bulanan * ter_rate

		print(f"DEBUG PPH: Total Pendapatan Bruto Bulanan: {total_pendapatan_bruto_bulanan}")
		print(f"DEBUG PPH: PPh 21 Bulanan (sebelum pembulatan): {pph21_bulanan}")

		return round(pph21_bulanan)

	except Exception as e:
		print(f"DEBUG PPH: Exception caught: {e}")
