...
def run():
	...
	print(f"--- Starting Employee ONLY Import for {company_name} ---")
	print(f"Reading from: {csv_file_path}")

	try:
		# Clear cache to ensure latest DB state is fetched
		frappe.clear_cache()
		...
		print("\n✨ Employee ONLY import finished successfully! ✨")
...
def process_employee_data(all_rows, company_name):
	...
	# Fetch Company and Gender names (primary keys) once
	company_doc_name = frappe.db.get_value("Company", {"company_name": company_name}, "name")
	if not company_doc_name:
		raise frappe.DoesNotExistError(
			f"Company '{company_name}' not found in database. Please create it manually."
		)

	gender_map = {}
	for gender_display_name in ["Laki - Laki", "Perempuan"]:
		gender_doc_name = frappe.db.get_value("Gender", {"gender": gender_display_name}, "name")
		if not gender_doc_name:
			raise frappe.DoesNotExistError(
				f"Gender '{gender_display_name}' not found in database. Please create it manually."
			)
		gender_map[gender_display_name] = gender_doc_name
	...
	# Mapping for Blood Groups
	blood_group_map = {
		"A": "A+",  # Assuming A means A+
		"B": "B+",  # Assuming B means B+
		"": "",  # Empty string is allowed
		"O": "O+",  # Assuming O means O+
		"A+": "A+",
		"A-": "A-",
		"B+": "B-",
		"B-": "B-",
	...
	for i, row in enumerate(all_rows):
		...
		try:
			...
			gender_display_name = row[16].strip() if len(row) > 16 else None
			address = row[18].strip() if len(row) > 18 else None
			...
			education_level_csv = row[23].strip() if len(row) > 23 else None
			school, major = (row[24].strip(), row[25].strip()) if len(row) > 25 else (None, None)
			...
			blood_group_csv = row[36].strip() if len(row) > 36 else None
			npwp = row[40].strip() if len(row) > 40 else None

			if not nik or not full_name:
				print(f" -> Skipping row {i+2} due to missing NIK or Name.")
				continue

			# Validate and map Gender
			gender_frappe = gender_map.get(gender_display_name)
			if not gender_frappe:
				print(f" -> ❌ ERROR: Gender '{gender_display_name}' not found in map. Skipping employee.")
				continue

			# Validate and map Education Level
			education_level_frappe = education_level_map.get(education_level_csv)
			if education_level_csv and not education_level_frappe:
				print(f" -> ⚠️ WARNING: Education Level '{education_level_csv}' not mapped. Setting to None.")

			# Validate and map Blood Group
			blood_group_frappe = blood_group_map.get(blood_group_csv)
			if blood_group_csv and not blood_group_frappe:
				print(f" -> ⚠️ WARNING: Blood Group '{blood_group_csv}' not mapped. Setting to None.")

			# Validate Date of Birth
			if date_of_birth and date_of_birth > getdate():
				print(f" -> ⚠️ WARNING: Date of Birth '{date_of_birth}' is in the future. Skipping employee.")
				continue

			# user_id is NOT set in this script
			...
			if not frappe.db.exists("Employee", {"employee_number": nik}):
				...
				employee_doc = {
					"doctype": "Employee",
					"company": company_doc_name,  # Use fetched Company name
					"employee_number": nik,
					"employee_name": full_name,
					"first_name": full_name.split()[0],
					"status": "Active",
					"user_id": None,  # user_id is explicitly None here
					"department": section or department,
					"designation": jabatan,
					"employment_type": employment_type,
					"date_of_joining": date_of_joining,
					"date_of_birth": date_of_birth,
					"gender": gender_frappe,  # Use fetched Gender name
					...
					"blood_group": blood_group_frappe,
					...
					"custom_no_kk": no_kk,
					"education": [{"level": education_level_frappe, "school": school, "major": major}]
					if education_level_frappe and school
					else [],
				}
