...
EARNINGS_ORDER = [
	...
	"JP Perusahaan",
	"JKN Perusahaan",
	"Penyesuaian PKP PPh 21",  # Ditambahkan
]
...
ALL_COMPONENTS = {
	...
	"Penyesuaian PKP PPh 21": {  # Ditambahkan
		"type": "Earning",
		"abbr": "PKP-ADJ",
		"account": "1111.002 - Kas Besar - SIUMANG",
			...
			try:
				...
				doc.append(
					"accounts",
					{
						...
						"account": data["account"],  # Menggunakan akun dari data
					},
				)

				doc.amount_based_on_formula = data.get("amount_based_on_formula", 0)  # Set berdasarkan data
				doc.is_flexible_benefit = data.get("is_flexible_benefit", 0)  # Set berdasarkan data
				doc.statistical_component = data.get("statistical_component", 0)  # Set berdasarkan data
				doc.do_not_include_in_total = data.get("do_not_include_in_total", 0)  # Set berdasarkan data
...
def create_structures_per_employment_type(dry_run=False):
	...
	for emp_type in employment_types:
		# Skip jika employment_type kosong
...
def assign_structures_to_employees(dry_run=False):
	...
	for emp in employees:
		...
		if not frappe.db.exists("Salary Structure", structure_name):
			print(
				f"[WARN] Salary Structure '{structure_name}' untuk '{emp.employment_type}' tidak ditemukan. Melewati karyawan '{emp.employee_name}'."
			)
			continue
		...
		existing_ssa = frappe.db.get_value(
			"Salary Structure Assignment",
			{
				...
				"from_date": emp.date_of_joining,  # Harus cocok dengan date_of_joining
			},
			"name",
		)
